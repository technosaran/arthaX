      ...
      for (const inc of recurringIncomes) {
        if (isRecurrenceMatch(inc.recurrence_day)) {
          try {
            ...
            logger.error(`[Recurring Cron] Failed to process recurring income ${inc.id}:`, incErr);
          }
        }
      }
    }
      ...
      for (const exp of recurringExpenses) {
        if (isRecurrenceMatch(exp.recurrence_day)) {
          try {
            ...
            logger.error(`[Recurring Cron] Failed to process recurring expense ${exp.id}:`, expErr);
          }
        }
      }
    }
