...
export class DomainEventBus {
  ...
  public async publish<T = any>(eventName: string, payload: T, userId?: string): Promise<void> {
    ...
    const promises = Array.from(callbacks).map((cb) => {
      try {
        return Promise.resolve(cb(event)).catch((err) => {
          console.error(`[DomainEventBus] Async error handling event ${eventName}:`, err);
        });
      } catch (err) {
        console.error(`[DomainEventBus] Sync error handling event ${eventName}:`, err);
