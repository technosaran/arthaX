...
const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL || "https://ytqqzeemkzhifbtthanh.supabase.co";
...
async function runSeed() {
  ...
  if (profErr || !profiles || profiles.length === 0) {
    console.error("No user profile found:", profErr);
    process.exit(1);
  }

  const userId = profiles[0].id;
  console.log("Seeding interconnected dataset for user_id:", userId);
  ...
  console.log("✅ SUCCESS! Seeding completed successfully across all sections.");
}
